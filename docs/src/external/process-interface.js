  ...
  return function () {
    ...
    const ProcessManager = class {
      ...
      createProcess(base = {}, parentProcess = null) {
        const process = new processInterface.Process(this, parentProcess, base);
      ...
      _registerProcess(process) {
        if (!(process instanceof Process)) {
          throw new processInterface.ProcessError("Argument «process» must be an instance of «Process» in «_registerProcess»");
        }
      ...
      _unregisterProcess(pid) {
        if (this.processes[pid]) {
          ...
          throw new processInterface.ProcessError(`Cannot unregister non-existent process with PID: ${pid}`);
        }
      }
    };
    ...
    const Process = class {
      ...
      createSubprocess(base = {}) {
        if (this.state === "closed") {
          throw new processInterface.ProcessError(`Cannot create subprocess. Process ${this.pid} is already closed`);
        }
      ...
      close() {
        if (this.state === "closed") {
          throw new processInterface.ProcessError(`Process ${this.pid} is already closed`);
        }
        this.state = "closed";
